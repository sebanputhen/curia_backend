...
async function getAllFamilies(req, res) {
  try {
    // const families = await Family.find({ koottayma: req.params.koottaymaid })
      ...
      koottayma: req.params.koottaymaid,
      status: 'active' 
    })
...
const getBulkFamilyData = async (req, res) => {
  try {
    ...
    const families = await Family.find({ 
      ...
      .populate('parish', 'name shortCode')
      .populate('koottayma', 'name')
      .lean();
...
async function updateFamily(req, res) {
  try {
    ...
    if (family.status === 'deleted') {
      return res.status(400).json({ message: "Cannot update a deleted family." });
    }
...
async function deleteFamily(req, res) {
  try {
      const family = await Family.findByIdAndDelete(req.params.familyid);
    if (!family) {
      res.status(404).json({ message: "Family not found." });
    } else {
