...
// Helper function to get current financial year (April-March cycle)
...
async function getLatestTransaction(req, res) {
  try {
    ...
    const transaction = await Transaction.findOne({
      ...
      date: {
        $gte: new Date(`${currentFinancialYear}-04-01`),  // Start of financial year (April 1st)
        $lte: new Date(`${currentFinancialYear + 1}-03-31`)  // End of financial year (March 31st next year)
      }
    })
...
async function transferTransaction(req, res) {
  try {
    ...
    const existingTransaction = await Transaction.findOne({
      person: fromPerson,
      date: {
        $gte: new Date(`${currentFinancialYear}-04-01`),
        $lte: new Date(`${currentFinancialYear + 1}-03-31`)
      }
    });
...
const getKoottaymaWiseTitheInfo = async (req, res) => {
  try {
    const { parishId } = req.params;
    const currentFinancialYear = getCurrentFinancialYear();
    const startDate = new Date(`${currentFinancialYear}-04-01`);
    const endDate = new Date(`${currentFinancialYear + 1}-03-31`);
